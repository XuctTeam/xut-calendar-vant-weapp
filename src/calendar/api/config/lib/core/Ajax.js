...
export default function Ajax(defaultConfig) {
  // 挂载当前实例配置
  ajax.config = Object.freeze(typeof defaultConfig === 'object' ? merge(defaultConfig) : defaultConfig)
  ...
  function ajax(url, data, config) {
    ...
    // 将请求拦截遍历添加到链前面
    ajax.interceptors.request.forEach(({ fulfilled, rejected }) => chain.unshift(fulfilled, rejected), true)

    // 将响应拦截遍历添加到链后面
    ajax.interceptors.response.forEach(({ fulfilled, rejected }) => chain.push(fulfilled, interceptCancel(rejected)), false)
