/*
 * @Author: Derek Xu
 * @Date: 2022-06-20 16:37:16
 * @LastEditors: Derek Xu
 * @LastEditTime: 2023-10-18 09:41:04
 * @FilePath: \xut-calendar-vant-weapp\src\calendar\constants\images.ts
 * @Description:
 *
 * Copyright (c) 2022 by 楚恬商行, All Rights Reserved.
 */

import { DEFAULT_IMAGES_SERVER } from './url'

export default {
  /** 默认头像 */
  DEFAULT_AVATAR: DEFAULT_IMAGES_SERVER + 'group1/M00/00/00/CgAQDGKEgb2AE6fzAAB0mLZqMmI800.png',
  /** 登录背景图标 */
  DEFAULT_LOG_IMAGE: DEFAULT_IMAGES_SERVER + 'group1/M00/00/00/CgAQDGUvN56AC6djAAAIK7UIn8g381.png',
  /** 客服二维码 */
  DEFAULT_QR_IMAGE: DEFAULT_IMAGES_SERVER + 'group1/M00/00/00/CgAQDGKGB9iAIInkAAH5qvJoHLs183.png',
  /** 邀请分享二维码 */
  DEFAULT_ATTEND_BACKGROUND: DEFAULT_IMAGES_SERVER + 'group1/M00/00/00/CgAQDGKVeDSAGalcAADOrzkbMvs223.png',

  GROUP_ADD: DEFAULT_IMAGES_SERVER + 'group1/M00/00/00/CgAQDGKwT62ASKrrAAAFnqNFSFE176.png',

  GROUP_SEARCH: DEFAULT_IMAGES_SERVER + 'group1/M00/00/00/CgAQDGKwT9qASeuMAAAGmf3CT0c489.png'
}
