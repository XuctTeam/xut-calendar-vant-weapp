...
import { IMenuButton } from '@/calendar/store/store'
    ...
    if (menuButton) {
      if (sysInfo) {
        setStore({
          ...
          top: menuButton.top,
          statusBarHeight: sysInfo.statusBarHeight ? sysInfo.statusBarHeight : menuButton.top - 4
        })
        calendar.$cache.cacheSet({
          key: 'menuButton',
          data: {
            ...menuButton,
            marginRight: sysInfo.screenWidth - menuButton.right
...
export function setMenuButtonAsync(setStore: SetterOrUpdater<IMenuButton>) {
  calendar.$cache.cacheGet({ key: 'menuButton' }).then((mb) => {
    calendar.$cache.cacheGet({ key: 'sysInfo' }).then((si) => {
      if (mb && si) {
        setStore({
          precise: true,
          bottom: mb.bottom,
...
export const base64 = (str: string): string => {
  ...
  // 对编码的字符串转化base64
  const base64 = base64_encode(encode)
  return base64
}
