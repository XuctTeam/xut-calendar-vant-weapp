...
import { AtCalendarBodyListGroup, AtCalendarBodyProps, AtCalendarBodyState, Calendar } from '../calendar'
import { delayQuerySelector } from '../common/utils'
import { generateCalendarGroup, generateCalendarWeekGroup } from '../common/helper'
...
export default class AtCalendarBody extends React.Component<AtCalendarBodyProps, Readonly<AtCalendarBodyState>> {
  ...
  public constructor(props: AtCalendarBodyProps) {
    super(props)
    const { validDates, marks, format, minDate, maxDate, generateDate, selectedDate, selectedDates, monday, view } = props
    ...
    const listGroup = this.getGroups(generateDate, selectedDate, view)
  ...
  public UNSAFE_componentWillReceiveProps(nextProps: AtCalendarBodyProps): void {
    const { validDates, marks, format, minDate, maxDate, generateDate, selectedDate, selectedDates, monday, view } = nextProps
    this.generateFunc =
      view === 'month'
        ? generateCalendarGroup({
            validDates,
  ...
  private getGroups = (generateDate: number, selectedDate: Calendar.SelectedDate, view: 'month' | 'week'): AtCalendarBodyListGroup => {
    const dayjsDate = dayjs(generateDate)
    const arr: AtCalendarBodyListGroup = []
    const preList: Calendar.ListInfo<Calendar.Item> = this.generateFunc(dayjsDate.subtract(1, view).valueOf(), selectedDate)

    const nowList: Calendar.ListInfo<Calendar.Item> = this.generateFunc(generateDate, selectedDate, true)

    const nextList: Calendar.ListInfo<Calendar.Item> = this.generateFunc(dayjsDate.add(1, view).valueOf(), selectedDate)
  ...
  private animateMoveSlide = (offset: number, callback?: any): void => {
    this.setState(
      {
        isAnimate: true
      },
  ...
  private handleSwipeTouchStart = (e: ITouchEvent & { changedTouches: Array<ITouch> }): void => {
    const { clientY, clientX } = e.changedTouches[0] ?? { clientX: 0, clientY: 0 }
  ...
  public render(): JSX.Element {
    const { isSwiper, monday, lunar } = this.props
    ...
    if (!isSwiper) {
      return (
        <View className={classnames('main', 'at-calendar-slider__main', `at-calendar-slider__main--${process.env.TARO_ENV}`)}>
          <AtCalendarDayList monday={monday} />
          <View className='main__body body'>
            <View className='body__slider body__slider--now'>
              <AtCalendarDateList lunar={lunar} list={listGroup[1]?.list ?? []} onClick={this.props.onDayClick} onLongClick={this.props.onLongClick} />
            </View>
          </View>
        </View>
      )
        ...
        >
          ...
          >
            <View className='body__slider body__slider--pre'>
              <AtCalendarDateList list={listGroup[0]?.list ?? []} lunar={lunar} />
            </View>
            <View className='body__slider body__slider--now'>
              <AtCalendarDateList list={listGroup[1]?.list ?? []} lunar={lunar} onClick={this.props.onDayClick} onLongClick={this.props.onLongClick} />
            </View>
            <View className='body__slider body__slider--next'>
              <AtCalendarDateList list={listGroup[2]?.list ?? []} lunar={lunar} />
            </View>
          </View>
        </View>
      )
