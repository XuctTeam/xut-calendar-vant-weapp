...
import { CalendarDateInfo, CustomStyles, StyleGeneratorParams } from '../days/index'
...
const Day: FC<IProps> = (args) => {
  ...
  useEffect(() => {
    const lunarDayInfo = mode === 'lunar' ? CalendarTools.solar2lunar(value.fullDateStr) : null
  ...
  let lunarDayInfo = mode === 'lunar' ? CalendarTools.solar2lunar(value.fullDateStr) : null
  ...
  return (
    <View
      onLongPress={onDayLongPress ? () => onDayLongPress({ value: value.fullDateStr }) : undefined}
        ...
        <View className={lunarClassName.join(' ')} style={customStyles.lunarStyle}>
          ...
          {(() => {
            ...
            if (lunarDayInfo.IDayCn === '初一') {
              ...
              //@ts-ignore
              dateStr = lunarDayInfo.isTerm ? lunarDayInfo.Term : lunarDayInfo.IDayCn
            }
            return dateStr
          })()}
        </View>
