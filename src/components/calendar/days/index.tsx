...
const Days: FunctionComponent<DaysProps> = ({
  ...
  const extraInfoDateList = extraInfo ? extraInfo.map((value) => value.value) : []
  const endDateStr = selectedRange ? selectedRange.end : ''
  ...
  return (
    <View className='calendar-body' style={bodyStyle}>
      {days.map((value, index) => {
        ...
        const disable =
          new Date(value.fullDateStr).getTime() < minDateObj.getTime() || (maxDate && new Date(value.fullDateStr).getTime() > maxDateObj.getTime()) || false
        return (
          <Day
            key={index}
