...
import { eventCenter, getCurrentInstance, showToast, useDidShow } from '@tarojs/taro'
...
function Render(props: IProps & { ctx: any; menuButton: IMenuButton }): any {
  ...
  useEffect(() => {
    ...
    if (ctx.error && (ctx.error.code === LOGIN_CODE || ctx.error.code === 'JSError' || ctx.error.code === 'BoundaryError')) {
      if (!ctx.uniteConfig.page) {
        const ins = getCurrentInstance()
  ...
  if (ctx.error && (ctx.error.code === 'JSError' || ctx.error.code === 'BoundaryError')) {
    if (ctx.uniteConfig.page) {
...
export default function Index(props: IProps) {
  ...
  // 页面的初始化和组件的初始化要区分开来
  const enablePagePullDownRefresh = props.enablePagePullDownRefresh ?? (ctx.uniteConfig.page ? true : false)
  ...
  useEffect(function () {
    ...
    if (process.env.NODE_ENV === 'development') {
      const ins = getCurrentInstance()
      if (ins.page?.config?.navigationBarTitleText) console.warn('useNav为true的时候不要在配置文件设置navigationBarTitleText，默认为页面为true')
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
