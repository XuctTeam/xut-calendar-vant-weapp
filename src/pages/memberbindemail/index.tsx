 ...
 * @LastEditTime: 2022-08-07 22:53:28
...
import { back, useBack } from '@/utils/taro'
import './index.less'
...
export default Unite(
  {
    ...
    async onReady() {
      if (this.hooks['emailAuth']) {
        this.setState({
          email: this.hooks['emailAuth'].username
        })
      }
    },
  ...
  function ({ state, events }) {
    ...
    return (
      ...
      >
        <View className='box'>
          <CellGroup inset>
            <Field label='邮箱' placeholder='请输入邮箱' value={email} onChange={(e) => setEmail(e.detail)}></Field>
            <Field
              label='验证码'
              placeholder='请输入验证码'
              value={smsCode}
              ...
              renderButton={
                <Button size='small' type='primary' onClick={() => sendSmsCode()} disabled={disable}>
                  {smsText}
                </Button>
              }
            ></Field>
