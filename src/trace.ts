 ...
 * @LastEditTime: 2022-07-21 09:35:56
 * @FilePath: \xut-calendar-vant-weapp\src\trace.ts
 * @Description:
 *
...
const { exposure, log, monitor } = Trace(
  {
    ...
    request(type /** log｜monitor */, data) {
      data = (data as any[])?.filter((item) => !/(redirectTo:fail)|(hideLoading:fail)|(cancel)/.test(item.d1))
